...
Starting from the entry point (user action, incoming request, WS tick), trace every transformation the data undergoes until it reaches its destination (UI render, DB write, outbound response). Draw it as a linear pipeline:
...
Count the layers. Three layers for a request handler is normal; seven is a smell.
