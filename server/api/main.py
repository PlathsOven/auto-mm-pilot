...
@app.get("/api/transforms", response_model=TransformListResponse)
...
@app.patch("/api/transforms", response_model=TransformListResponse)
async def update_transforms(req: TransformConfigRequest) -> TransformListResponse:
    ...
    if stream_configs:
        try:
            await asyncio.to_thread(rerun_pipeline, stream_configs, transform_config=full_config)
            await restart_ticker()
        except Exception as exc:
