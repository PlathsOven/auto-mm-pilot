...
Covers everything that crosses the non-LLM API boundary: stream CRUD,
snapshot ingest, block table, transform configuration, aggregate market
values, the WebSocket broadcast payload, and pipeline time-series
responses. Imports ``_shared`` + stdlib + pydantic only — never reaches
into ``auth`` or ``llm``.
"""
...
from server.api.models._shared import SnapshotRow, _WireModel
...
# Forward ref resolution for ClientWsMarketValueFrame.entries.
ClientWsMarketValueFrame.model_rebuild()
