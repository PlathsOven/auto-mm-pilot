"""Temporal-fair-value transforms — per-timestamp fair value for each block.

Market-implied fair value is no longer a per-block concept; it is computed
at the space level in ``market_value_inference`` and combined in the
aggregation step. Each block here only emits its own ``fair`` curve.
"""
...
def fv_standard(
    ...
    """Exact port of the original compute_block_fair_values logic (fair only)."""
    ...
    for row in blocks_df.iter_rows(named=True):
        ...
        total_val = _get_total_value(target_val, start_ts, end_ts, is_ann)
