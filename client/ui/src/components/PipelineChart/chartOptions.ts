...
// ---------------------------------------------------------------------------
// Per-space decomposition builder
// ---------------------------------------------------------------------------

/** Pick the calc-space array on a per-space block for the given metric. */
function resolveSpaceMetric(s: SpaceSeries, metric: Metric): number[] {
  switch (metric) {
    case "fair":       return s.fair;
    case "variance":   return s.var;
    case "marketCalc": return s.marketFair;
    default:           return [];
  }
}

/**
 * Build a stacked per-space decomposition option for ``metric``.
 *
 * Uses a **category** x-axis (per ``tasks/lessons.md`` — ECharts ``stack``
 * requires category, not time). Each space_id becomes one stacked line
 * whose values sum additively to ``total_*_calc`` at every timestamp
 * (calc space is variance-linear). Axis labels reuse the shared local-time
 * formatter so the decomposed view reads the same as the single-metric
 * view.
 */
export function buildPipelineDecomposedOptions(
  data: PipelineTimeSeriesResponse,
  metric: Metric,
): EChartsOption {
  const timestamps = data.aggregated.timestamps;
  const perSpace = data.aggregated.perSpace;
  ...
  const seriesSpecs: EChartsOption["series"] = spaceIds.map((id, i) => ({
    name: id,
    type: "line",
    stack: "decomp",
    areaStyle: { opacity: 0.18 },
    data: resolveSpaceMetric(perSpace[id], metric),
    showSymbol: false,
    lineStyle: { width: 1.2, color: BLOCK_COLORS[i % BLOCK_COLORS.length] },
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (paramsRaw) => {
        const params = Array.isArray(paramsRaw) ? paramsRaw : [paramsRaw];
        if (params.length === 0) return "";
        const iso = String(params[0].name ?? "");
        const d = parseIsoUtc(iso);
        const header = d ? formatTooltipDate(d) : iso;
        let runningSum = 0;
        const rows = params
          .map((p) => {
            ...
            return `<div style="display:flex;justify-content:space-between;gap:16px;">`
              ...
              + `<span style="font-family:monospace;">${sci(v)}</span></div>`;
          })
          .join("");
        const total = `<div style="display:flex;justify-content:space-between;gap:16px;margin-top:4px;padding-top:3px;border-top:1px solid rgba(0,0,0,0.08);font-weight:600;">`
          + `<span>${metricLabel[metric]} total (calc)</span>`
          + `<span style="font-family:monospace;">${sci(runningSum)}</span></div>`;
        return `<div style="font-weight:600;margin-bottom:4px;">${header}</div>${rows}${total}`;
      },
    },
    dataZoom: [
      { type: "inside", filterMode: "filter" },
    ...
    xAxis: {
      ...
      axisLabel: {
        ...
        hideOverlap: true,
        formatter: (v: string) => {
          const d = parseIsoUtc(v);
          return d ? tickFormatter(d.getTime()) : v;
        },
      },
      axisTick: { lineStyle: { color: "rgba(0,0,0,0.08)" } },
      axisLine: { lineStyle: { color: "rgba(0,0,0,0.08)" } },
