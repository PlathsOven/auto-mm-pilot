...
- `AGENTS.md` — architecture, lane ownership, visibility barrier, key files
- `.windsurfrules` — agent behavior rules, the "Manual Brain" restriction
- `STACK_STATUS.md` — which components are PROD / MOCK / STUB / OFF
...
- [ ] **God files** (>200 lines doing multiple things) — split by responsibility
...
**Execution rules:**
- **NEVER modify `server/core/`** — document needed changes, skip
...
```bash
npx --prefix client/ui tsc --noEmit 2>&1 | head -50
```
...
If refactoring changed:
- File locations or names → update `AGENTS.md` Key Files table + `README.md` directory tree
- Data flow or pipeline steps → update MVP Pipeline in `AGENTS.md`
- Component status → update `STACK_STATUS.md`
- New canonical patterns → add a brief note to `.windsurfrules` so future agents follow them

---

### 9. Commit

One surgical commit per phase:

```bash
.cascade/commands/commit-push-pr.sh "refactor(phase1): delete dead weight — orphaned files, debug leftovers" <files...>
.cascade/commands/commit-push-pr.sh "refactor(phase2): canonicalize patterns — <list elected patterns>" <files...>
.cascade/commands/commit-push-pr.sh "refactor(phase3): vectorize scalar loops in <lane>" <files...>
.cascade/commands/commit-push-pr.sh "refactor(phase4): decompose god files, extract shared types" <files...>
.cascade/commands/commit-push-pr.sh "refactor(phase5): rename to domain vocabulary, add type hints" <files...>
```

Wait for approval before executing each commit.
