...
- `docs/user-journey.md` — every change ultimately lands in front of the trader or the operator. Re-read the persona and the flow the current task touches, so you can frame "what does the user see when this ships?"
...
- **Verification** — how you will know it worked (typecheck, manual test, etc.)
- **Risks / open questions** — anything that would change the plan if answered differently

If the task touches >3 files or crosses the API boundary, delegate to `/preflight` before proposing the plan.
