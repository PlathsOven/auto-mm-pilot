...
## 2026-04-24 — Expiry correlation calculator library (method 1: forward-addition α-blend)

**Context:** The Stage H expiry correlation matrix ships as a manually edited upper-triangle in the Anatomy CorrelationsEditor. For k expiries, that's `k(k−1)/2` numbers the trader has to eyeball. A trader who wants "close expiries highly correlated, far expiries less so" with a single tunable knob should not have to hand-author a 20×20 matrix. We want to carry forward a *library of calculation methods* that derive the full draft upper-triangle from a small param set, and ship method 1 as the canonical term-structure-shaped default.

**Decision:** Add `server/api/correlation_calculators/` — a Protocol + registry + one calculator module. First method is **`forward_addition_blend`**, derived from the identity `E_later = E_earlier + ForwardExpiry_{earlier,later}` under standard Brownian-path variance scaling (σ ∝ √T): at ρ=0 the algebra collapses to the classic `√(T_short/T_long)` term structure; ρ=1 forces every pair to 1; the method exposes a single α ∈ [0, 1] linearly blending the two extremes. Two endpoints added to `routers/correlations.py`: `GET /api/correlations/expiries/methods` (returns the library + param schema for the UI picker) and `POST /api/correlations/expiries/apply-method` (runs a named calculator, writes entries to the draft slot via the existing `set_draft`). Confirm / Discard gate promotion — applying a method never commits, and the existing dirty-flag + singular-matrix checks still fire on promotion. UI picker deferred to a follow-up.

**Rationale:** (1) **Semantic choice of σ is load-bearing.** We considered σ = std of the vol itself ("vol of vol"), with σ ∝ 1/T as the natural "long-dated IVs are more stable" scaling. Under the correct variance-weighted IV composition law that framing collapses to `√((1+ρ)/2)` — a *flat* correlation that's independent of expiries, which isn't what we want. σ = std of *cumulative returns* (the BS framing) gives σ ∝ √T from the Brownian-path identity, the classic term-structure shape falls out, and the model is consistent with the pricing pipeline's own flat-vol assumption (`fair_value.py` uses `σ²·T` accumulated variance). (2) **One-knob → full-matrix is the right UX.** Any calculator with more than ~1 param defeats the purpose — the trader would still be tuning a matrix, just via a different interface. The library lets future methods add params if they must, but method 1 stays single-knob. (3) **Library scaffolding matters before method 2.** A flat module would have been faster now but made method 2 a refactor. The `ExpiryCorrelationCalculator` Protocol + `registry.py` + schema metadata means a new method drops into the library with zero router / model changes. (4) **Draft-only, not commit.** Applying a method fills the draft and surfaces the same Confirm/Discard flow the manual editor already has. This preserves the "commits are deliberate" invariant and reuses the singular-matrix check on promotion. (5) **Canonical-ISO + tenor floor.** Calculator consumes expiries via the same `canonical_expiry_key` the store uses, so `DDMMMYY` / tz-aware ISO / naive ISO all round-trip consistently. `MIN_YEARS_TO_EXPIRY = 1/(365.25·24)` (≈ 1 hour) floors the denominator so an elapsed or at-tick expiry doesn't divide by zero.

**Consequences:** New public API: `GET /api/correlations/expiries/methods` → `ExpiryCorrelationMethodsResponse` (per-method name + label + param schema), `POST /api/correlations/expiries/apply-method` (takes `{method_name, params, expiries}`, writes draft, returns the usual `ExpiryCorrelationListResponse`). New Pydantic models: `ApplyExpiryCorrelationMethodRequest`, `ExpiryCorrelationMethodParam`, `ExpiryCorrelationMethodSchema`, `ExpiryCorrelationMethodsResponse`. New server package: `server/api/correlation_calculators/` (`__init__.py`, `base.py`, `forward_addition_blend.py`, `registry.py`). 14 unit tests in `server/tests/test_forward_addition_blend.py`. New TS types mirroring the server models in `client/ui/src/types.ts`; API helpers in `correlationsApi.ts`; `useCorrelationsDraft` grows a nullable `applyMethod` (null for symbols, wired for expiries); new `MethodPicker` component (dropdown + per-param slider + Apply button) mounted above the expiry section in `CorrelationsEditor`. The existing manual-edit surface stays available — the calculator writes the draft, but the trader can still click cells in the `MatrixGrid` to override individual entries before confirming. Follow-ups (not in this change): (a) second calculator method when the trader's needs outgrow a single-knob term structure; (b) symbol-correlation calculator library (same pattern) if the same one-knob pressure emerges there.
