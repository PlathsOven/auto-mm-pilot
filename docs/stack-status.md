...
| **Correlation Store** | `server/api/correlation_store.py` | `PROD` | — | Per-user symbol + expiry committed/draft stores mirroring `MarketValueStore`; dirty flag flipped by draft PUT / Confirm / Discard; consumed by `engine_state.rerun_pipeline` + `ws._check_dirty_rerun`. |
| **Correlation Matrix Helpers** | `server/api/correlation_matrix.py` | `PROD` | numpy | Pure functions: `materialise_matrix(entries, labels)` + `check_singular(matrix, kind)` + typed `SingularCorrelationError(matrix_kind, det, condition_number)`. Stage H's failure channel. |
| **Correlation Alert Store** | `server/api/correlation_alert_store.py` | `PROD` | Correlation Matrix Helpers | Latest singular-matrix alert per user per kind; populated by engine_state's rerun catch + cleared on success; read by the WS ticker into `ServerPayload.correlation_alerts`. |
| **Correlation Endpoints** | `server/api/routers/correlations.py` | `PROD` | Correlation Store | Eight routes under `/api/correlations/{symbols,expiries}/{,draft,confirm,discard}`; expiry labels canonicalised to ISO before Pydantic validation. |
| **Stage H — Exposure → Position** | `server/core/transforms/exposure_to_position.py` | `PROD` | numpy, Correlation Matrix Helpers | Per-timestamp `P = Cₛ⁻¹·E·Cₑ⁻¹` via vectorised numpy solve over `(T, k, m)`. Identity-matrix default is a no-op; draft path writes nullable `*_hypothetical` columns. Raises `SingularCorrelationError` on `|det| < 1e-9` — no Tikhonov fallback. |
...
| **Anatomy DAG** | `client/ui/src/components/studio/anatomy/` | `PROD` | TransformsProvider, stream API | React Flow canvas — stream nodes (hover for details, click to edit), `+ New stream` tile, transform nodes, Correlations node (Stage H), output node |
| **Correlations Editor** | `client/ui/src/components/studio/correlations/` (`CorrelationsEditor.tsx`, `MatrixGrid.tsx`, `ConfirmMatrixModal.tsx`) | `PROD` | `useCorrelationsDraft`, WebSocketProvider | Two-matrix editor hosted in Anatomy Correlations node panel — live Σ\|Δ\| preview from WS `*_hypothetical` columns, per-matrix Confirm/Discard, loud per-cell diff modal at confirm time. |
| **Correlations API Client** | `client/ui/src/services/correlationsApi.ts` (+ `hooks/useCorrelationsDraft.ts`) | `PROD` | FastAPI correlations router | Thin `apiFetch` wrappers + hook owning committed/draft state with 500ms debounced PUT + generation-counter cancellation. |
