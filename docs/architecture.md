...
| `server/api/routers/correlations.py` | 8 routes under `/api/correlations/{symbols,expiries}/{,draft,confirm,discard}`. Expiry labels canonicalised to ISO before Pydantic validation. |
...
| `client/ui/src/components/studio/correlations/CorrelationsEditor.tsx` | Stage H control surface — stacks symbol + expiry `MatrixGrid` editors, live Σ\|Δ\| summary off WS `*_hypothetical` columns, Confirm/Discard per matrix. |
