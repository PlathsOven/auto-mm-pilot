...
3. **Inspect.** Clicks the cell → workbench focus snaps to it → the right-rail Inspector channels to that (symbol, expiry): edge / variance / desired position scalars, the top contributing blocks, and an "Ask @Posit" affordance. Clicking a row header focuses the symbol; clicking a column header focuses the expiry; clicking a row in the bottom Opinions panel focuses that opinion and surfaces its editable description, live chart, and derived-blocks summary. The Blocks tab collapses multi-dim fan-outs into block families (e.g. one `ema_iv` row covering BTC × ETH × 4 expiries), with an expand chevron for per-dim drill-down. Opinions committed via Build mode surface the immutable original phrasing from the LLM trace alongside the editable trader description. No chat opens unless the trader explicitly asks for it — inspection and conversation are separate gestures.
...
5. **Review impact.** The new stream appears as a row in the Opinions panel (Data-source pill). The customer opens the Pipeline Chart + clicks the opinion's row to see how its blocks change edge, variance, and desired positions across symbols and expiries — the inspector shows the live chart, any Build-orchestrator concerns, and the derived-blocks summary. The Blocks tab gives the per-dim numeric drill-down if needed.
6. **Iterate.** If the impact looks wrong — positions moved too much, or not enough — he adjusts parameters (confidence weight, decay, scale) with LLM guidance and reviews again. The opinion's editable description is updated inline so the desk shares context on what the stream represents.
...
3. **Create the opinion.** Build mode emits a `create_manual_block` engine command, which the client routes to the BlockDrawer (titled "New Opinion") for review. Once the customer submits the form, the opinion appears in the Opinions panel with the View pill and his natural-language phrasing as the description.
4. **Review impact.** Same as Flow 2, step 5 — click the opinion row to see the inspector; the chart + derived-blocks summary confirm how the view flows through the pipeline.
5. **Adjust or remove.** If the view changes or the event passes, the customer edits the description inline (original LLM phrasing stays preserved underneath for audit), toggles the opinion inactive to hide it from the pipeline without losing the data, or deletes it via the trash-confirm affordance. The `+ New opinion` button in the Opinions panel header is the authoring shortcut when the trader wants to skip the LLM mediation and enter a block directly.
